...
(async()=>{
 ...
 await page.getByText('Live review · assessments use retrieved material only.',{exact:true}).waitFor();
 assert.equal(await page.evaluate(()=>window.INJECTED),undefined);
 assert.equal(await page.locator('.claim img').count(),0);
 await page.getByRole('button',{name:'Assess retrieved evidence',exact:true}).click();
 await page.getByText('No readable evidence was retrieved for this claim.',{exact:false}).waitFor();
 assert.equal(await page.locator('.quote').count(),0);
 await page.getByRole('button',{name:'Extract claims with AI',exact:true}).click();
 await page.waitForFunction(()=>document.querySelector('#status').textContent.includes('GEMINI_API_KEY'));
 await page.getByText('Edit this claim',{exact:true}).click();
 await page.getByLabel('Edit claim text',{exact:true}).fill('An edited synthetic claim.');
 await page.getByRole('button',{name:'Apply claim edit',exact:true}).click();
 await page.getByRole('heading',{name:'An edited synthetic claim.',exact:true}).waitFor();
 await page.setViewportSize({width:390,height:844});
 await page.screenshot({path:'test-results/live-mobile.png',fullPage:true});
 assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>window.innerWidth),false);
 assert.deepEqual(errors,[]);
 console.log('Browser smoke passed: empty/demo/live, edited/rejected decisions, JSON provenance, missing evidence/key, claim edits, safe text rendering, mobile overflow; no page errors.');
