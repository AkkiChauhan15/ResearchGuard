"""Run one bounded synthetic extraction and assessment with the selected provider.

This script never changes billing, selects a fallback, prints source passage bodies,
or displays an API key.
"""
...
def main() -> int:
    ...
    source = Source(
        ...
        category="synthetic",
        title="Synthetic public-safe provider verification passage",
    ...
    try:
        ...
        print(json.dumps({
            ...
            "note": "Retries are bounded inside each task. No fallback or demonstration result was used.",
        }, indent=2))
        return 2
    ...
    print(json.dumps({
        "status": "passed",
        "provider": status.provider,
        "free_access_gate": "passed server configuration checks",
        "requested_models": {
