...
| Non-Gemini evidence-provider boundary | Fixture and public-source payload verified | Groq, OpenRouter-free and NVIDIA request shapes, structured parsing and safe errors pass controlled tests; the exact 53-passage PMC record was reduced from a 43,056-byte full payload to an 11,581-byte model payload with 8 exact excerpts while the canonical source stayed complete |
...
| Retained Gemini compatibility | Fixture and bounded live synthetic generation verified | Earlier requests returned HTTP 503; on 2026-09-26 one extraction and one assessment completed with `gemini-3.8-flash`, structured/Pydantic checks, source-ID and exact-quotation validation; Gemini is not the hosted default |
